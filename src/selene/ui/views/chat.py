...
def _init_chat_state() -> None:
    ...
    logger.debug(
        "_init_chat_state: session_id=%s chat_history=%d persisted=%d",
...
def render_chat() -> None:
    ...
    _init_chat_state()
    logger.debug(
        "render_chat: ENTER session_id=%s history_count=%d",
        st.session_state.get("chat_session_id"),
    ...
    if col2.button("+ New", use_container_width=True):
        logger.info("render_chat: + New clicked, clearing current session")
        clear_current_session()
        st.rerun()
    ...
    if prompt := st.chat_input("Ask about symptoms, HRT, or research..."):
        logger.info("render_chat: received user prompt len=%d", len(prompt))
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("Thinking..."):
                ...
                logger.debug(
                    "render_chat: contextualized query changed=%s",
                    search_query != prompt,
                )
                ...
                logger.debug(
                    "render_chat: rag retrieval context_len=%d sources=%d",
                    len(context or ""),
                    len(sources or []),
                )
                ...
                logger.debug(
                    ...
                    len(chat_res),
                    len(relevant_chats),
                    Config.CHAT_HISTORY_DISTANCE_THRESHOLD,
                )
                if relevant_chats:
                    # Filter and Truncate
                    formatted_past = []
            ...
            resp_container.markdown(full_response)
            logger.info(
                "render_chat: generated response len=%d sources=%d related_chats=%d",
                len(full_response),
                len(sources or []),
                len(relevant_chats),
            )
